function dataSet = importAndParseDataSimple(edfData, locChannel,rocChannel,startREM,endREM)
...
psgTemp1L = passFilter(edfData.record(locChannelNum,startREM:endREM),settings.SAMPLE_RATE,settings.LOW_FILTER_CUTOFF_FREQ,'low',settings.FILTER_ATENUATION); %20Hz cutoff filter
psgTemp1R = passFilter(edfData.record(rocChannelNum,startREM:endREM),settings.SAMPLE_RATE,settings.LOW_FILTER_CUTOFF_FREQ,'low',settings.FILTER_ATENUATION);
...
%% Tidy and printout
% save important vars
dataSet.rawTimeData = [edfData.record(locChannelNum,startREM:endREM) ; edfData.record(rocChannelNum,startREM:endREM)];
